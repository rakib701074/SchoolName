                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <!-- <a href="{{ url('/student/delete') }}/{{ $student->student_id }}" onclick="confirmation(event)">
                                                    <button class="btn btn-danger" title="delete" data-id="">
                                                        <i class="fa fa-trash-o"></i>
                                                    </button></a> -->


                                                <a href="#" class="skill" data-id="{{ $teachers->teacher_id }}">
                                                    <button class="btn btn-danger" title="delete" data-id="">
                                                        <i class="fa fa-trash-o"></i>
                                                    </button></a>
                                            </td>
...
<script>
    $(document).ready(function(){
        $('.skill').on('click',function(){
            var studentID = $('.skill').attr('data-id');
            data = {
                'CSRFToken' : getCSRFTokenValue(),
                id: studentID,
            }
            var url = "{{url('/student/delete')}}"
            customAjaxCall(url, data, studentResponse)
        })

        function studentResponse(input) {
            if(input.status) {
                modal(input.status, input.message)
            }
        }
    })
</script>
